...
The CIDAS Policy Engine lets security leads encode per-project package-safety rules in a versioned JSON file (`.cidas/policy.json`) that lives alongside the source code. Unlike ad-hoc `.npmrc` deny lists or network-level firewalls, CIDAS policies travel with the repository, are version-controlled with the code they protect, and take unconditional precedence over all pillar scores — including the per-machine trust list. This makes CIDAS the first npm screening tool with a code-native, hierarchically-resolved, three-layer policy mechanism.

## 2. Policy file format

Policy files must conform to `.cidas/policy.schema.json`. Every field is optional; an empty `{}` is a valid (no-op) policy. Files with unknown keys or wrong value types are silently discarded (safe fallback), and the `policy_file` field in the scan response is set to `null` to signal that no policy was applied.
